...
//   const { user } = React.useContext(Authorization); 2
...
//       <ScheduleData.Provider value={{ loading }}>
//         <ScrollView>
//           <SegmentedButtons
//             value={value}
...
export default function ScheduleScreen({ navigation, route }) {
  ...
  const AScreen = () => {
    return (
      <View>
        {
          loading ? <Loading /> : bookingHistory.length > 0 ? bookingHistory.map((item) => {
            return (
              <ScheduleHistory item={item} navigation={navigation}
                key={item.id}
              />
  ...
  const BScreen = () => {
    return (
      <View>
        {
          loading ? <Loading /> : bookingFinished.length > 0 ? bookingFinished.map((item) => {

            return (
              <ScheduleFinish item={item} navigation={navigation} key={item.id} />
            )
          }) : <NotData message="You don't have an appointment yet" />
        }
      </View>
  ...
  return (
    <Tab.Navigator>
      <Tab.Screen name="Lịch sử" component={AScreen} />
      <Tab.Screen name="Hoàn Thành" component={BScreen} />
    </Tab.Navigator>
  )
}